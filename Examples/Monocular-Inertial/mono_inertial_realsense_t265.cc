...
int main(int argc, char** argv) {
    ...
    auto imu_callback = [&](const rs2::frame& frame) {
        ...
        if (rs2::frameset fs = frame.as<rs2::frameset>()) {
            ...
            if (m_frame.get_profile().stream_name() == "Gyro") {
                ...
                while (v_gyro_timestamp.size() >
                       v_accel_timestamp_sync.size()) {
                    int index = v_accel_timestamp_sync.size();
                    double target_time = v_gyro_timestamp[index];

                    rs2_vector interp_data = interpolateMeasure(
                        target_time, current_accel_data,
    ...
    while (!SLAM.isShutDown()) {
        ...
        for (int i = 0; i < vGyro.size(); ++i) {
            ...
            if (isnan(vAccel[i].x) || isnan(vAccel[i].y) ||
                isnan(vAccel[i].z) || isnan(vGyro[i].x) || isnan(vGyro[i].y) ||
        ...
        double timeProcess = std::chrono::duration_cast<
                                 std::chrono::duration<double, std::milli> >(
                                 t_Start_Track - time_Start_Process)
                                 .count();
        double timeSLAM = std::chrono::duration_cast<
                              std::chrono::duration<double, std::milli> >(
                              t_End_Track - t_Start_Track)
                              .count();
